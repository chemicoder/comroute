...
export default function Map({
  ...
  const pathFetchKeyRef = useRef<Record<string, string>>({});
  ...
  useEffect(() => {
    ...
    if (route.stops && route.stops.length > 1) {
      ...
      const recent = route.history.slice(-50);
      coordsString = recent.map((h) => `${h.lng},${h.lat}`).join(';');
    }

    if (!coordsString) return;
    const key = `${route.id}:${coordsString}`;
    if (pathFetchKeyRef.current[route.id] === key) return;
    pathFetchKeyRef.current[route.id] = key;
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (data.routes && data.routes[0]) {
          ...
          setRoadPaths((prev) => ({ ...prev, [route.id]: coords }));
        }
      } catch {
        // silent fallback
      }
  ...
  }, [selectedRoute?.id, selectedRoute?.stops, selectedRoute?.history]);
  ...
  const BusIconColored = (isSelected: boolean, color: string) =>
    L.divIcon({
    ...
    <div className={`w-full h-full relative ${isPinpointing ? 'cursor-crosshair' : ''}`}>
      ...
      >
                  ...
                  <Popup>
                    <div className="p-2 min-w-[200px]">
                      ...
                      <div className="flex items-center justify-between">
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${route.type === 'public' ? 'bg-green-100 text-green-700' : 'bg-purple-100 text-purple-700'}`}
                        >
                          {route.type}
                        </span>
        ...
        {routes.map((route) => {
          ...
          const historyCoords =
            route.history?.map((h) => [h.lat, h.lng] as [number, number]) || [];
            ...
            <div key={`path-${route.id}`}>
              {roadPath && isSelected && (
                <>
                  <Polyline
                    positions={roadPath}
                    pathOptions={{ color: routeColor, weight: 12, opacity: 0.2 }}
                    eventHandlers={{
                      click: (e) => onPolylineClick?.(route.id, e.latlng.lat, e.latlng.lng),
                    }}
                  />
                  <Polyline
                    positions={roadPath}
                    pathOptions={{ color: routeColor, weight: 8, opacity: 0.4 }}
                    eventHandlers={{
                      click: (e) => onPolylineClick?.(route.id, e.latlng.lat, e.latlng.lng),
                    }}
                  />
                  <Polyline
                    positions={roadPath}
                    pathOptions={{ color: routeColor, weight: 4, opacity: 1 }}
                    eventHandlers={{
                      click: (e) => onPolylineClick?.(route.id, e.latlng.lat, e.latlng.lng),
                    }}
                  />
              ...
              {!roadPath && historyCoords.length > 1 && isSelected && (
                <Polyline
                  ...
                  pathOptions={{ color: routeColor, weight: 4, opacity: 0.6, dashArray: '10, 10' }}
                  eventHandlers={{
                    click: (e) => onPolylineClick?.(route.id, e.latlng.lat, e.latlng.lng),
                  }}
                />
